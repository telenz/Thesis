...
These searches were interpreted within an anomaly-mediated Supersymmetry breaking model~\cite{FIXME} with $\tan\beta=5$ and $\mu<0$.
The excluded parameter space by these searches is shown in Fig.~\ref{fig:ATLAS}.
Parameter regions ...

\begin{figure}[!t]
  \centering
      \includegraphics[width=0.49\textwidth]{figures/theory/mass_adlo_gaug_1.pdf}
  \caption{Observed and expected exclusion limits in the $m_{\chipm}-\Delta m$ plane for almost degenerate $M_1$ and $M_2$ and a large nified scalar mass $m_0$. Taken from~\cite{bib:LEP:SUSY_results}.}  
  \label{fig:LEP}
\end{figure}
\begin{figure}[!b]
  \centering
      \includegraphics[width=0.60\textwidth]{figures/theory/ATLAS_SUSY_LLPChargino.pdf}
  \caption{. Taken from~\cite{FIXME}.}  
  \label{fig:ATLAS}
\end{figure}


\subsection*{Searches at CMS at 7 and 8\tev}
Mention all searches.
Only show DT plot.
