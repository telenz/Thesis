...
The formulation of a relativistic quantum field theory and of spontaneous symmetry breaking (SSB) by the Brout-Englert-Higgs mechanism, allowed to built a theory which is capable to explain almost all observations of particle physics at colliders until today~\cite{bib:Theory:GFitter}.
...
\end{equation}
Consequently, the masses of the gauge bosons are the following
\begin{equation}
 \begin{split}
  M_H =\,&  \sqrt{2} \mu\\
  M_W =\,& \frac{g}{\sqrt{2}}\, v\\
...
A phase space suppression is possible when the mass splitting between the decayed particle and one of the decay products is very small.
...
In the MSSM, the lightest chargino (\chipm) and the lightest neutralino (\chiO) can be almost mass-degenerate, when the wino mass parameter ($M_2$) is smaller than the bino ($M_1$) and higgsino ($\mu$) mass parameters.
This can be seen from the chargino and neutralino mass matrices.
The chargino mass matrix in the basis $\Psi^+_i= \left(-i \tilde{W}^+,\tilde{h}_u^+  \right)$ and $\Psi^-_i= \left(-i \tilde{W}^-,\tilde{h}_d^-  \right)$ is given by 
\begin{flalign}
&\mathcal{M}_{\Psi^{\pm}} = 
\begin{pmatrix} 
M_2    & g v_d                    \\
...
\end{pmatrix}.&
\end{flalign} 
The mass eigenstates can be deduced with the help of orthoganal matrices $V$ and $U$, $\chi^+_i=V_{ij}\Psi^+_j$, $\chi^-_i = U_{ij} \Psi^-_j$.
The neutralino mass matrix in the basis  $\Psi_i^0= \left(-i\tilde{B},-i\tilde{W}^0,\tilde{h}_u^0,\tilde{h}_d^0, \tilde{s}^0\right)$ is
\begin{flalign}
&\mathcal{M}_{\Psi^0} = 
\begin{pmatrix} 
...
\end{pmatrix}.&
\end{flalign}
In first order approximation - neglecting the off-diagonal elements which are of electroweak strength - the lightest chargino and the lightest neutralino are both wino-like for $M_2 < M_1,\mu$ with a mass of $m_\chi = M_2$. 
Thus, the mass difference between \chipm and \chiO is only determined by higher order corrections (radiative corrections as well as tree-level mixing with other states)loop contributions, making the mass splitting naturally very small.
...
Furthermore, recent analyses of the pMSSM parameter space~\cite{bib:pMSSMScan_2013,bib:pMSSMScan_2012} show, that almost pure wino-like neutralinos are very frequent for flat scans in the pMSSM parameter space and that they mostly come along with wino-like charginos being the next-to leading lightest supersymmetric particle (NLSP).
For these scenarios, their mass splitting is typically of the order of $\sim160\mev$~\cite{bib:pMSSMScan_2013}.
...
%Additionally, constraints from indirect searches are summarised.

%\subsection*{Searches at HERA}
...
\begin{figure}[!t]
  ...
  \caption{Observed and expected exclusion limits in the $m_{\chipm}-\Delta m \left( \chipm, \chiO \right)$ plane for almost degenerate $M_1$ and $M_2$ and a large unified scalar mass $m_0$. Taken from~\cite{bib:LEP:SUSY_results}.}  
  \label{fig:LEP}
\end{figure}
...
\begin{figure}[!b]
  ...
  \caption{Excluded parameter space in the $m_{\chipm}-\tau_{\chipm}$ plane for an AMSB model with $\tan\beta=5$ and $\mu>0$. Only chargino pair production is taken into account. The area below the curves is excluded. Taken from~\cite{bib:ATLAS_SUMMARYPLOTS}.}  
  \label{fig:ATLAS}
\end{figure}
...
The disappearing track search by CMS shows are very similar sensitivity as the searches done at the ATLAS experiment.
\begin{figure}[!h]
  \centering
      \includegraphics[width=0.54\textwidth]{figures/theory/lifetimeNs_vs_mass.pdf}
  \caption{Excluded parameter space in the $\tau_{\chipm}-m_{\chipm}$ plane for wino-like charginos. The region left to the curve is excluded. Taken from~\cite{bib:CMS:DT_8TeV}.}  
  \label{fig:CMS}
\end{figure}

\subsection*{Indirect searches}
Finally, also results from indirect dark matter searches constrain the SUSY parameter region of wino-like charginos and neutralinos.
