...
The most popular breaking mediation mechanisms is gravity-mediated Supersymmetry breaking~\cite{bib:GravityMediation} and gauge-mediated supersymmetry breaking~\cite{bib:GaugeMediation}.   

%FIXME, maybe talk about AMSB models.

\chapter{Long-lived particles in the MSSM}
\label{ch:Longlived_Particles}
There are various mechanisms how particles can be long-lived, such as small couplings or supressed decays because of high mediator masses (weak interaction).
...
A phase space suppression is possible when the mass splitting between the decay particle and one of the decay products is very small.
...
The underlying mechanism of this mass-degeneracy will be addressed in the next paragraphs.
...
This can be seen in the masses of the interaction eigenstates.
...
Thus, the mass differences between \chipm and \chiO is only determined by higher loop contributions, making the mass splitting naturally very small.

Furthermore, recent analyses of the pMSSM parameter space~\cite{bib:pMSSMScan_2013,bib:pMSSMScan_2012} show, that almost pure wino-like neutralinos often come along with wino-like charginos and their mass splitting is typically of the order of $\sim160\mev$~\cite{bib:pMSSMScan_2013}.
...
At the ATLAS experiment at the LHC, searches for events with a dissapearing track signature were conducted at $\sqrt{s}=7\tev$~\cite{bib:PreviousSearches_Atlas_DT_7TeV} as well as at $\sqrt{s}=8\tev$~\cite{bib:PreviousSearches_Atlas_DT_8TeV}. 
Furthermore, a search for metastable particles with high ionisation loss was performed with $\sqrt{s}=8\tev$ data~\cite{bib:PreviousSearches_ATLAS_DEDX}.
...
\begin{figure}[!t]
  ...
  \caption{Observed and expected exclusion limits in the $m_{\chipm}-\Delta m \left( \chipm, \chiO \right)$ plane for almost degenerate $M_1$ and $M_2$ and a large nified scalar mass $m_0$. Taken from~\cite{bib:LEP:SUSY_results}.}  
  \label{fig:LEP}
\end{figure}
\begin{figure}[!b]
  \centering
...
Among them is the search for long-lived charged particles~\cite{bib:CMS:HSCP_8TeV}, which searched for heavy particles with large energy deposits in the tracker.
