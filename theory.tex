...
Furthermore, it is $C_1=1 $ for doublets and $C_1=0$ für singlets under $SU(2)_L$, $C_2=1$ for triplets and $C_2=0 $ for singlets under $SU(3)_C$.  
...
First of all, the Standard Model suffers of the so-called hierachy problem.
By this, the problem is adressed that the higgs mass is not protected from getting large radiative corrections to the bare mass ($m_{h^0}^{\text{bare}\,2}$).
...
The most strinking shortcoming of the Standard Model is the missing fourth fundamental force, the gravitanional force.
Within the SM, it is not possible to add renormalisable terms, that can describe the gravitational force.
Although, gravity is not important for particle physics at energies that are accesible at current particle colliders (it only becomes important at the Planck scale $\sim 10^{19}\gev$), the fact that it cannot be embedded into the Standard Model leads to an understanding of the SM as effective theory, only valid for lower energies.
...
The share of non-visible matter to the total amount of energy in the universe is estimated to be $~27\%$~\cite{FIXME}.
In comparison, baryonic matter contributes only by $\sim5\%$ to the total amount of energy in the universe.
...
Finally, the so-calles baryon-antibaryon assymetry is not explainable within the SM.
The only source for CP-violation, which is needed to give rise to a matter-antimmater assymetry is contained via the CKM matrix, which appears in the SM due to the weak interaction in the quark sector.
The complex phase in this matrix is however a tiny number, not able to explain such an assymetry.\\


There are far more questions, not all adreesedd.
The reader is refererred to~\cite{} to get a much detailed overview about the limitations of the SM.

In the following Section~\ref{sec:Supersymmetry}, a theory is introduced, that can adrees most of the above mentioned probelm.
...
Supersymmetry was developed by xxx.
...
Now explain why they can be lon-lived.
