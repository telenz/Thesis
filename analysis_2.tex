...
\begin{table}[h]
\centering
\caption{Simulated signal mass points with corresponding cross sections the NLO-NLL (NLO: next-to-leading order, NLL: next-to-leading logarithmic accuracy) for wino-like charginos.}
...
If ISR occurs, it is possible to trigger on a high-\pt jet and missing transverse energy.
...
%\end{table}  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Analysis selection/ Selection of signal candidate events}
...
\item The position of the vertex along the beam line must be within 24\cm from the beam origin (FIXME: bunch crossing): \mbox{$|dz| \leq 24\cm$.}
\item The position in the transverse direction must be within 2\cm from the beam origin (FIXME): \mbox{$|d0| \leq 2\cm$.}
\end{itemize}
...
\end{itemize}
These requirements result in a trigger efficiency of xxx at the cut threshold~\cite{FIXME}.\\
...
A general list of dead and noisy ECAL cells is provided centrally at CMS~\cite{FIXME}.
Further dead cells were identified within a study in~\cite{bib:FIXME} resulting in a total number of 1234 dead or noisy ECAL channels. 
...
\begin{figure}[!t]
  ...
  \end{tabular}
  \caption{Visualisation of dead and noisy ECAL cells in the detector's $\phi - \eta$ plane according to~\cite{bib:FIXME}.}
...
\begin{table}[!hbt]
\centering
\caption{Intermodule ECAL gaps \cite{FIXME}.}
...
Thus, also tracks pointing towards these regions within a distance of $\Delta R<0.25$ are rejected. FIXME. Show the list. Cite something


To summarise, the candidate track must fulfil the following selection criteria:
FIXME: Einheitliche veto bullets
\begin{itemize}
\renewcommand{\labelitemi}{\footnotesize{\ding{118}}}
...
\item No jet ($\pt>20\gev$ and $|\eta|<4.5$) is allowed around the track in a cone of $\Delta R< 0.5$
...
FiXME: As emphasised before, this analysis aims at being sensitive to different chargino lifetimes  to search for charginos with different lifetimes.
To allow for charginos decaying at any layer of the tracker, no explicit selection cut on the number of missing outer hits is required.
