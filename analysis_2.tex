...
In the following two sections an overview is given about the SM (Sec.~\ref{sec:SMSamples}) and SUSY samples (Sec.~\ref{sec:SignalSamples}) used to design the search.
...
It only limits the possibility of a comparison between data and simulation going beyond shape comparisons.\footnote{E.g. the background process \ZInvJets sample is not available. It can possibly contribute via the fake background to the search. However, as the rate of fake tracks is independent of the underlying process, this background can be studied with the same quality with a \WJets sample.}
...
A derivation of this formula can be found in Appendix~\ref{blabla}.
...
To maximise the signal acceptance, the trigger related selection cuts are chosen as close as possible to the trigger thresholds (see Sec.~\ref{sec:DatasetsAndTriggers}).
...
\subsection{Track based selection}
...
These selection requirements are of course highly suppressing the background emerging from real lepton production as in \WJets events.
The discrimination power of the lepton vetos is shown in Fig.~\ref{FIXME}.
...
\begin{figure}[!t]
  ...
  \end{tabular}
  \caption{Track isolation (left) and calorimeter energy deposits (right) of the candidate track after the full previous selection (after all other selection cuts of the candidate track selection).}
...
The optimisation process will be explained in Sec~\ref{sec:Optimisation}.
