...
public:
  ...
  void Selection(bool isSR, bool doMatching, double ptCut, double ecaloCut){
	...
	histoASmiSR   -> Fill(trackCaloIso->at(i),weight_xsec_lumi*weight);
	histoASmi     -> Fill(trackCaloIso->at(i),weight_xsec_lumi*weight);
      }
    }
...
int makeFakeIasCRSRplots(){
  ...
  //pred.file = new TFile("/nfs/dust/cms/user/tlenz/ANALYSIS/workdir/analysis_2015_06_15_METGt0_JetPtGt70_trackPtGt20_d0CutNotInverted_FakeCS/results/analyzer/ntuples/input_weighted/wjets.root","READ");
  ...
  Double_t xbinsASmiSR[3]  = {0,20,100};
  Double_t xbinsASmi[6]  = {0,2.5,5,10,50,100};
  //Double_t xbinsASmi[5]  = {0,0.1,0.2,0.4,1};
  pred.histoASmiSR   = new TH1D("MC_CR","MC CR",2,xbinsASmiSR);
  pred.histoASmi     = new TH1D("MC_CR","MC CR",5,xbinsASmi);
  SR.histoASmiSR     = new TH1D("MC_SR","MC SR",2,xbinsASmiSR);
  SR.histoASmi       = new TH1D("MC_SR","MC SR",5,xbinsASmi);
  cout<<"1"<<endl;
  pred.Selection(0,1,ptCut,ecaloCut);
  SR.Selection(1,1,ptCut,ecaloCut);
  cout<<"###########################################################################"<<endl;
  ...
  TCanvas *c2 = drawRatioPlotWithPurity(pred.histoASmiSR,SR.histoASmiSR,"dE/dx discriminator (I_{as})",filename + pred.histoASmi->GetName() + "_"  + SR.histoASmi->GetName() + ".pdf" ,pred.purity,ptCut);
