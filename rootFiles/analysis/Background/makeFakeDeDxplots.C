...
public:
  ...
  void Selection(bool doMatching, int pdgId, bool lowCaloRegion){
	...
	histo     -> Fill(trackASmi->at(i),weight);
      }
      
    }

    histo->GetXaxis()->SetTitle("dE/dx discriminator (I_{as})");
...
int makeFakeDeDxplots(){
  ...
  sample muons;
  sample elecs;
  sample fakesLowECalo;
  sample fakesHighECalo;
  
  //TString selection  = "chiTrackspreselectionNoTrigger";
  //TString pathTofile = "~/PlotsForThesis/VBF-LS-tau-tools/drawHist/input_Met100_trackPtGt20_3/";

  TString selection  = "chiTrackspreselectionNoTrigger";
  TString pathTofile = "/nfs/dust/cms/user/tlenz/ANALYSIS/workdir/analysis_2015_08_19_METGt0_JetPt70_FakeCS/results/analyzer/ntuples/";
  ...
  fakesLowECalo.file    = new TFile(pathTofile + "/input_weighted/wjets.root","READ");
  fakesLowECalo.file     -> GetObject(selection + "/Variables",fakesLowECalo.tree);
  fakesHighECalo.file    = new TFile(pathTofile + "/input_weighted/wjets.root","READ");
  fakesHighECalo.file     -> GetObject(selection + "/Variables",fakesHighECalo.tree);

  pions.getTreeVariables();
  muons.getTreeVariables();
  elecs.getTreeVariables();
  fakesLowECalo.getTreeVariables();
  fakesHighECalo.getTreeVariables();
  ...
  */
  ...
  pions.Selection(1,211,1);
  muons.Selection(1,13,1);
  elecs.Selection(1,11,1);
  fakesLowECalo.Selection(1,0,1);
  fakesHighECalo.Selection(1,0,0);

  pions.histo->Scale(1./pions.histo->Integral());
  muons.histo->Scale(1./muons.histo->Integral());
  elecs.histo->Scale(1./elecs.histo->Integral());
  fakesLowECalo.histo->Scale(1./fakesLowECalo.histo->Integral());
  fakesHighECalo.histo->Scale(1./fakesHighECalo.histo->Integral());
  ...
  fakesLowECalo.histo->SetLineColor(kBlack);
  fakesHighECalo.histo->SetLineColor(kRed);

  pions.histo->SetMarkerColor(kPink-3);
  muons.histo->SetMarkerColor(kGreen -6);
  elecs.histo->SetMarkerColor(867);
  fakesLowECalo.histo->SetMarkerColor(kBlack);
  fakesHighECalo.histo->SetMarkerColor(kRed);

  muons.histo->SetMarkerStyle(21);
  fakesLowECalo.histo->SetMarkerStyle(24);
  fakesHighECalo.histo->SetMarkerStyle(24);
  ...
  fakesLowECalo.histo->SetMinimum(min);
  fakesLowECalo.histo->SetMaximum(max);    


  pions.histo->Draw();
  muons.histo->Draw("same");
  elecs.histo->Draw("same");
  fakesLowECalo.histo->Draw("same");
  ...
  delete legend;
  legend =  new TLegend(0.25,0.73,0.9,0.9);
  legend->SetTextSize(0.05);
  legend ->AddEntry(fakesLowECalo.histo ,"Fakes (E_{calo}^{#DeltaR<0.5} < 10GeV)","lep");
  legend ->AddEntry(fakesHighECalo.histo,"Fakes (E_{calo}^{#DeltaR<0.5} > 10GeV)","lep");
  fakesLowECalo.histo->Draw();
  fakesHighECalo.histo->Draw("same");
  legend->Draw("same");
  text -> Draw("same");
