...
Unfortunately,  the unknown mechanism of supersymmetry breaking, the most general parametrization of Supersymmetry introduces over 100 new dimensions which opens up an incredibly huge phenomenalogically rich space, 
...
The problem is caused by the underlying probability distribution of a single $\Delta E/ \Delta x $ measurement: the Landau distribution \cite{bib:Landau_1944}
\begin{equation}
p(x) = \frac{1}{\pi} \int_0^\infty\! e^{-t \log t - x t} \sin(\pi t)\, dt.
\label{eq:landau}
\end{equation}
%Even if widely used, theBethe-Bloch formula is ill-defined because of the use of the mean energy loss per path length.
...
In the following section the method of the gain calibration of the pixel silicon tracker explained. 
...
For every module in the pixel tracker (there are 1440 modules in total), each $\Delta E/\Delta x$ measurement of all particles crossing the module is filled into a histogram. 
...
\begin{equation}
c_{\text{inter}}=\frac{MPV [\text{ADC/mm}]}{300 \cdot 265 [\text{ADC/mm}]}.
\end{equation}
...
Finally the overall calibration factor $c_{\text{strip}}$ is determined.


%subsubsection*{Validation}
%There were several validation procedures conducted to ensure that the calibration dis the right thing.
%First of all

\subsubsection*{Results}
The final result is $1440\cdot5$ different calibration factors $c_{\text{inter}}$.
These can be found in Fig.~\ref{fig:CalibrationFactors} for all five different time steps.
...
\begin{figure}[!bt]
  \centering 
  \begin{tabular}{c}
    \includegraphics[width=0.31\textwidth]{figures/analysis/PixelGain_AB_NEW.pdf}
    \includegraphics[width=0.31\textwidth]{figures/analysis/PixelGain_C1_NEW.pdf}
    \includegraphics[width=0.31\textwidth]{figures/analysis/PixelGain_C2_NEW.pdf}
  \end{tabular}
  \begin{tabular}{c}
    \includegraphics[width=0.31\textwidth]{figures/analysis/PixelGain_D1_NEW.pdf}
    \includegraphics[width=0.31\textwidth]{figures/analysis/PixelGain_D2_NEW.pdf}
  \end{tabular}
  \caption{The resulting calibration factors for all five time step (from left top to bottom right in increasing time).} 
  \label{fig:CalibrationFactors}
\end{figure}
