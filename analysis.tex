...
\begin{figure}[!b]
  \centering 
  \begin{tabular}{c}
  \includegraphics[width=0.6\textwidth]{figures/analysis/Landau.pdf}
  \end{tabular}
...
\begin{figure}[!b]
  \centering 
  \begin{tabular}{c}
  \includegraphics[width=0.6\textwidth]{figures/analysis/StabilityPlot_Pixel_beforeCalibration_withoutStepFits_NEW.pdf}
  \end{tabular}
...
\begin{figure}[!bt]
  \centering 
  \begin{tabular}{c}
  \includegraphics[width=0.6\textwidth]{figures/analysis/Landau_Module_352476680.pdf}
  \end{tabular}
...
\begin{figure}[!bt]
  \centering 
  \begin{tabular}{c}
  \includegraphics[width=0.6\textwidth]{figures/analysis/StabilityPlot_Pixel_afterCalibration_withoutStepFits_NEW.pdf}
  \end{tabular}
...
\begin{figure}[!bt]
  \centering 
  \begin{tabular}{c}
  \includegraphics[width=0.6\textwidth]{figures/analysis/StabilityPlot_Strip_afterCalibration_withoutStepFits_NEW.pdf}
  \end{tabular}
...
That is, determining the probability that the set of all $\Delta E/ \Delta x$ belonging to one track is actually compatible with the hypothetical probability distribution of a MIP.
Testing that a measured sample has been drawn from a specific distribution is know as the Cram\'{e}r-von Mises test \cite{bib:Anderson:CramerVonMises_1962}.
The so-called Smirnov-Cram\'{e}r-von Mises discriminator \cite{blabla} is a discriminator, which is not sensitive to lower or higher values
\begin{equation*}
I_s = \frac{3}{N} \cdot \left( \frac{1}{12N} + \sum\limits_{h=1}^N \left[ P_h - \frac{2h-1}{2N} \right]^2 \right).
\end{equation*}
Therefore in \cite{bib:Quertenmont_2010} it was a so-called Asymmetric Smirnov-Cram\'{e}r-von Mises discriminator developed, which is only sensitive to incompatibilites to the MIP hypothesis towards higher energy depositions
\begin{equation*}
I_{as} = \frac{3}{N} \cdot \left( \frac{1}{12N} + \sum\limits_{h=1}^N \left[ P_h \cdot \left(P_h - \frac{2h-1}{2N} \right)^2 \right] \right).
\end{equation*}

The underlying propability distribution of the MIP hypothesis was extracted from the same ``Minimum bias'' sample (plus the selection specific for MIPs) used for the pixel energy calibration.
The probability distribution of the energy deposition for the silicon strip detector was provided by ...



\begin{itemize}
\item Assymetric Smirnov Discriminator
\item Cite paper for comp exp and theoretically dis
\item Show a plot of the underlying prop distribution
\item Not a huge section
\item Show the templates
\item Show also distributions of assymetric smirnov discriminator (compare signal to SM)
\end{itemize}
