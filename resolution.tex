...
% FIXME: It is done because response distributions with a scale smaller than one are typically narrower while distributions with scales larger than one are broader.
...
FIXME: The observed difference in the resolution measurement of the two approaches is taken as a systematic uncertainty on the measured resolution in data and propagated to the final data-to-simulation ratio.
...
FIXME: Another source of uncertainty is the use of information from Monte Carlo simulation by determining the residual imbalance $q$ from the simulated \GAMJET sample (Eq.~\eqref{eq:total}).
...
FIXME: For a future measurement, a reduction of the uncertainties is desirable.
