...
These lifetimes are further sutiable as they lie at the edge of the sensitivity of the search for disapperaing tracks~\cite{bib:CMS:DT_8TeV}.
...
\begin{figure}[!t]
  ...
  \end{tabular}
  \caption{Comparison of the 95\% CL exluded parameter region from~\cite{bib:CMS:DT_8TeV} (black line) and the excluded parameter region of this analysis (red line).
           The results are very similar. A slight improvement of the size of the excluded parameter region by this analysis can be seen.}
...
For a chargino with a lifetime of $\tau=0.07\ns$, the observed limit of this search improves the limits of~\cite{bib:CMS:DT_8TeV} by $\sim35\gev$, for a lifetime of $\tau=0.4ns$ by $\sim25\gev$. 
The confirmation of the excluded parameter space in~\cite{bib:CMS:DT_8TeV} is even more interesting as the confimation is achieved with a very uncorrelated selection.
The correlation of the signal region in~\cite{bib:CMS:DT_8TeV} and the most sensitive signal region (out of the four exclusive regions) of this analysis can be estimated by the correlation coefficient of two overlapping sets
\begin{equation*}
\rho_{\text{corr}} = \frac{N_{A\cap B}}{N_A + N_B - N_{A \cap B}}
\end{equation*}
The correlation coefficient for some of the examined signal model can be found in Table~\ref{FIXME}.
It can be seen, that for very low lifetimes, the overlap of the here preseneted search and the search for disappearing tracks~\cite{bib:CMS:DT_8TeV} is of minor importance.
...
The relic density is possible to explain with wino-like neutralinos that are non-thermally produced via the decay of long-lived particles, such as a wino-like chargino.\\

The presented analysis extends the search for disappearing tracks~\cite{bib:CMS:DT_8TeV}, by including the variable \dedx.
It is the first analysis at CMS that includes energy information from the pixel tracker.
By this inclusion the discrimantion power of \dedx is impressingly increased.
Furthermore, \dedx inclusion enables to loosen track selection requirements that lead to a high supression of signal events.
...
Exclusion on the SUSY parameter space is set.
The parameter exclusion limits in~\cite{bib:CMS:DT_8TeV} can be confirmed and slight improvements of th order $\sim 10-40\gev$ in the low lifetime region is achieved.
Interstingly, the signal regions of the here preseneted search and the search from~\cite{bib:CMS:DT_8TeV} are highly non-corrlated and thus this analysis can serve as an orthogonal cross check of~\cite{bib:CMS:DT_8TeV}.\\

The main background is arising from fake tracks, that are also typically short and can have large values of \ias.
Further improvements in the background supprresion can be possibly achieved by the characterisation of further unique characteristics fake tracks, such as ????.
