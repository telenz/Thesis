...
In order to include systematic uncertainties, the background expectation $b$ and the signal expectation $\mu\cdot s$ are varied within a Gaussian distribution with a width estimated as the 1-$\sigma$ uncertainty and a mean of zero.
...
This integration is generally not solvable analytically, but is calculated with pseudo data drawn from the Gaussian and inserted into the Poisson function.
...
\begin{figure}[!h]
  ...
  \label{fig:1dLimits}
\end{figure} 
The upper 95\%\,CL limit on the cross section is strongest for lifetimes between $10-100\cm$.
...
Charginos with a higher mass of 500\gev are excluded for lifetimes between $70-500\cm$.

Since the long lifetime of a wino-like chargino is caused by the small mass splitting between $m_{\chipm}$ and $m_{\chiO}$, 
...
The derived exclusion limits of this search could confirm the exclusion from~\cite{bib:CMS:DT_8TeV} with slight improvements in the low lifetime region.
The comparison of the two searches is shown in Fig~\ref{fig:Comparison2dLimit}.

For a chargino with a lifetime of $\tau=0.07\ns$, the observed limit of this search improves the limits derived inf~\cite{bib:CMS:DT_8TeV} by $\sim35\gev$, for a lifetime of $\tau=0.4\ns$ by $\sim25\gev$. 
For SUSY models with long chargino lifetimes the here presented search shows a higher exclusion power.
The weaker exclusion for long lifetimes from~\cite{bib:CMS:DT_8TeV} is caused by the additional selection cut in the number of missing outer hits, $\nlostouter\geq3$.

The confirmation of the excluded parameter space in~\cite{bib:CMS:DT_8TeV} is even more interesting as the confirmation is achieved with low correlation.
The correlation between the selected signal events of the signal region in~\cite{bib:CMS:DT_8TeV} and the most sensitive signal region (out of the four exclusive regions) of this analysis can be estimated by the event overlap $\rho_{\text{corr}}$
\begin{equation*}
\rho_{\text{corr}} = \frac{N_{A\cap B}}{N_A + N_B - N_{A \cap B}}
\end{equation*}
Only the most sensitive signal region is chosen in order not to underestimate the correlation by artificially adding other signal regions that are not important for the search sensitivity.
The degree of correlation is depicted in Fig.~\ref{fig:AnalysisOverlap} that shows the event overlap for signal models with chargino masses between $100-600\gev$ and lifetimes between $5\cm-1000\cm$.
\begin{figure}[!t]
  \centering 
  \begin{tabular}{c}
...
\end{figure} 
It can be seen, that for intermediate lifetimes around 100\cm, the event overlap is around 50\% and reduces for small lifetimes to small overlaps of around 15\%.
Thus, this analysis is able to confirm the exclusion limits derived in~\cite{bib:CMS:DT_8TeV} with a search that show low correlation for the selected signal events for low chargino lifetimes.
...
The relic density is possible to explain with wino-like neutralinos that are non-thermally produced via the decay of long-lived particles, such as a wino-like chargino~\cite{bib:Moroi:DarkMatter_2013}.
...
In order to increase the search sensitivity with respect to short lifetimes, also energy information from the pixel silicon tracker is taken into account.
...
It is thus the first analysis at CMS that made use of energy information from the pixel tracker.
By this inclusion the discrimination power of \dedx could be significantly increased.
Furthermore, \dedx inclusion enabled to loosen track selection requirements that led to a high suppression of signal events, such as the number of hits in the tracker system.
...
Predominantly, data-based background estimation techniques were used.
...
The low statistical precision constituted a serious challenging, as the study of background characteristics from different sources is mainly possible with simulated samples.
Therefore, non-correlated variables were identified that could therefore be loosened in order to increase the statistical power.

The main background to this search is arising from fake tracks, tracks that are reconstructed out of several particle's trajectory.
Fake tracks are typically short and can have large values of \ias, thus they show a very signal-like signature in the detector.
...
No excess above the Standard Model expectation is observed.
...
For high mass scenarios of 500\gev, the excluded lifetime ranges between $70-500\cm$.
Thus, the derived parameter exclusion limits in~\cite{bib:CMS:DT_8TeV} could be confirmed and slight improvements of the order $\sim 10-40\gev$ in the low lifetime region were achieved.
Interestingly, the signal regions of the here presented search and the search from~\cite{bib:CMS:DT_8TeV} are not heavily correlated.
Therefore, this analysis also serves as an independent cross check of~\cite{bib:CMS:DT_8TeV}.
...
Thus, the increase of the search sensitivity by including \ias will become even stronger for higher mass scenarios.

Up to now, no evidence for Supersymmetry could be found.
Still, such a explanatory powerful and aesthetically convincing theory will not be given up easily.
That makes especially exotic searches very interesting and it will remain (requisite) important to look also in the little corner of Supersymmetry in the future.
