...
FIXME: Think about writing about chi2 cut and the suppression of fake tracks.
...
The generation of simulated samples is done with the help of Monte Carlo (MC) methods (see \eg~\cite{FIXME}).
...
Since \madgraph is responsible for the simulation of well separated partons whereas \pythia is suppose to simulate the soft and collinear showers, the event is rejected if the simulated shower - clustered with the \kt method~\cite{FIXME} - has a too large radius parameter (MLM matching scheme~\cite{FIXME}).
...
\begin{figure}[!t]
  \centering
      \includegraphics[width=0.80\textwidth]{figures/experiment/Simulation/MCGeneration2}
