...
\item Iteration 1: Tracks with only two pixel hits and $\pt>0.8\gev$ are recovered. FIXME
...
\begin{figure}[!t]
  ...
  \caption{Parton-level event in the $\eta - \phi$ plane with $\sim 1000$ soft particles added. Three different algorithms are used to cluster the particles into jet cones (coloured areas): the Cambridge/Aachen algorithm (left), the \kt algorithm (middle), and the \antikt algorithm (right). Taken from~\cite{FIXME}.}  
...
Since, all previously applied calibration factors are determined on simulation, it cannot yet be ensured that data is equally well calibrated.
Therefore, additional correction factors are applied on data only (level-2 and \mbox{level-3} residual corrections).
...
\item \textbf{L2L3 residual corrections:} Finally, correction factors are determined in order to remove any residual bias in data. They are estimated in data with the help of QCD-multijet events to eliminate differences in $\eta$ (L2) and with $\gamma/Z+$ jets events in order to eliminate differences in \pt (L3).
...
The neutral pions decay into two photons with a probability of $\sim 99\%$. 
The photons in turn convert into electron pairs with a high probability.
...
The two analysis described in this thesis are subject to an event cleaning procedure as recommended by the CMS collaboration~\cite{FIXME internal twiki}.
...
In this thesis, all simulated samples are generated with the PDFs provided by the \cteq group~\cite{Pumplin:2002vw} FIXME: Check this.

Afterwards, the hard interaction between the selected partons is simulated, followed by the simulation of showering and hadronisation.
...
Since \madgraph is responsible for the simulation of well separated partons whereas \pythia is designed for simulating the soft and collinear showers, the event is rejected if the simulated shower - clustered with the \kt method (see Section~\ref{sec:JetReconstruction}) - has a too large radius parameter (MLM matching scheme~\cite{bib:MLM_matching}). FIXME: Is this correct.
