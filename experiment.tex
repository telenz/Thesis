...
The particle-flow algorithm proceeds as follows in an event (the description is based on~\cite{CMS-PAS-PFT-09-001}):
\begin{enumerate}
...
\begin{enumerate}
\item Each global muon (hits detected in the tracker as well as in the muon system) where the \pt measured in both sub-detectors is compatible with the tracker measurement only is defined as particle-flow muon and the track in both sub-detectors is removed from the block.
\item Electron reconstruction and identification follows using tracker hits and ECAL clusters. 
...
\item The compatibility of the remaining ECAL and HCAL energy deposits to the transverse momentum of the reconstructed tracks within a block is checked. 
      ...
      Only, if the energy deposits in the ECAL or HCAL are much larger than the corresponding track \pt, it gives rise to a particle-flow photon or particle-flow neutral hadron, respectively. 
      All used ECAL and HCAL clusters used for the identification as well as the reconstructed tracks are removed from the block.
...
A special task when reconstruction tracks consists in the suppression of fake tracks, \ie tracks that are not associated with a charged particle.
Therefore, the requirement of fulfilling certain quality criteria (fourth step) is crucial to substantially reduce the contamination of fake tracks.
 For this purposes, requirements on the following variables are imposed:
\begin{itemize}
...
\item $\chi^2/\text{ndof} < \alpha_0 N_{layers}$.
\item $|d_0^{\text{BS}}|/\delta d_0 < \left( \alpha_3 N_{layers} \right)^{\beta}$.
\item $|z_0^{\text{PV}}|/\delta z_0 < \left( \alpha_4 N_{layers} \right)^{\beta}$.
\item $|d_0^{\text{BS}}|/\sigma_{d_0} \left(\pt\right) < \left( \alpha_1 N_{layers} \right)^{\beta}$.
\item $|z_0^{\text{PV}}|/\sigma_{z_0} \left(\pt,\eta\right) < \left( \alpha_2 N_{layers} \right)^{\beta}$.
\end{itemize}
...
The parameters $\alpha$ and $\beta$ have in general different values in the various iterations.
...
Finally, in order to get rid of differences between data and simulation, jets measured in data are additional calibrated by residual correction factors (level-2 and \mbox{level-3} residual corrections).
...
The following description of the tau reconstruction algorithm follows~\cite{bib:CMS:TauReconstruction_8TeV}.

The HPS algorithm performs in two separate steps. 
...
The successful identification of a tau requires a reconstruction of a neutral pion which is present in most of the hadronic decays.
Since a neutral pion decays into two photons in 99\% which in turn convert into electron-pairs with a high probability, photon and electrons are clustered into strips in the $\eta-\phi$ plane in order to identify possible $\pi^0$.
The initial position of the strip equals the position of the seed electron or photon (photon or electron with the highest \pt not yet clustered into any strip).
...
After the reconstruction of the neutral pion, multiple decay modes hypothesis are tested by combining up to two clustered strips with one or three charged particles.
For all hypothesis, several restriction on the invariant mass of the hadronically decaying tau are applied.
Finally, the hypothesis fulfilling the invariant mass requirement and - in case this happens for more than one hypothesis - with a reconstructed tau with the highest \pt is chosen.

Furthermore, for all taus in this thesis, the discriminators which are used for the reduction of jet, electron and muon contamination are required to be less than zero (see~\cite{bib:CMS:TauReconstruction_8TeV} for further details on the discriminators).

Finally, a loose isolation criterion is demanded, which is calculated by the (pileup corrected) scalar sum of the transverse momentum of all charged particles and photons with a \pt larger than 0.5\gev in a cone of $R=0.5$ around the tau.
...
They serve as comparison to measured data and can give an estimation of the expected number of background events for a given selection.
...
In the following sections, a short introduction into the various steps of event simulation is given (see e.g.~\cite{bib:Matthias_Thesis} for a more thorough introduction).\\

The first step of the event generation of $pp$-collision events consists of the simulation of the hard process, \ie the primary interaction of two partons of the two colliding protons.
Which type of partons are involved in the primary interaction is specified by the so-called parton distribution functions (PDFs).
They describe the probability of a parton (up-, down-quarks, gluons, sea quarks) to carry a certain energy fraction of the proton energy and thus define the probability for each parton taking part at the hard interaction process.
In this thesis, all simulated samples make use of the PDFs provided by the \cteq group~\cite{Pumplin:2002vw} FIXME: Check this.


Afterwards, the generation of the matrix element as well as the simulation of showering and hadronisation is performed.
...
Since \madgraph is responsible for the simulation of well separated partons whereas \pythia is suppose to simulate the soft and collinear showers, the event is rejected if the simulated shower - clustered with the \kt method (see Section~\ref{sec:JetReconstruction}) - has a too large radius parameter (MLM matching scheme~\cite{bib:MLM_matching}).
...
Finally, the interaction of the traversing particles with the detector material is simulated with \geant~\cite{bib:Geant4_2003,bib:Geant4_2006} throughout this thesis.
