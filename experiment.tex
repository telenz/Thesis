...
The reconstruction of primary vertices is important in order to determine the location of the interaction point and to get an estimate of the corresponding uncertainty.
At the CMS experiment, the reconstruction of vertices proceeds as follows~\cite{bib:CMS:tracking_8TeV}:
First, a track selection is perfomered which depends on the number of hits in the tracker, on the significance of the transverse impact parameters, and the normalised $\chi^2$ of the track fit.
...
For each tracks is, a weight is assigned that is close to one in case for a good compatibility and close to zero for a bad compatibiltiy with the vertex.
The track with the largest sum of all squared track transverse momenta is referred to as the primary vertex. 
All other vertices during a bunch-crossing are so-called pileup interactions.

Further quality criteria of the primary vertex can be required.
...
Furthermore, the vertex is required to be within 24\cm in $z$- and 2\cm in $r$-direction.
...
In the barrel region a so-called ``hybrid'' algorithm is used which proceeds by adding in $\phi$ direction fixed arrays of $5 \times 1$ crystals in $\eta \times \phi$ if their energy deposists are larger than a certain minimal energy.
