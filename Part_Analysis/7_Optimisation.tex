...
This is achieved by a minimisation of the cross section for which a 5$\sigma$-discovery (significance $Z=5$) of the corresponding signal model is expected, \ie finding the optimal selection cuts for \pt and \ias for which the lowest possible cross section, $\sigma_{\text{min}}$, can be discovered
\begin{align}
...
Z = \frac{\alpha_{\text{min}} \cdot N_S(\text{mass},\ctau,\pt^{\text{cut}}, \ias^{\text{cut}})}{\Delta B (\pt^{\text{cut}}, \ias^{\text{cut}})} = 5. && \text{with   } \alpha_{\text{min}} = \frac{\sigma_{\text{min}}}{\sigma_{S}}.
\end{align}
...
\begin{table}[!h]
\centering
\caption{Optimal \pt and \ias selection cuts and the corresponding minimum cross section $\sigma_{\text{min}}$ that can be discovered with 5$\sigma$ significance for signal models with different chargino masses and lifetimes.
...
\multicolumn{5}{c}{} \\
\toprule
Mass [\gev] & Lifetime \ctau [\cm] & Optimal \pt cut [\gev] & Optimal \ias cut & $\sigma_{\text{min}}$ [\pb] \\
\midrule
...
For simplicity, general systematic uncertainties on the leptonic and the fake background of 100\% and 20\% respectively are used for the visualisation.
