...
In Fig.~\ref{fig:TriggerTurnOn}, the trigger turn-on curves, measured in data and in simulation (90\% \WJets events) are depicted.
\begin{figure}[!b]
  \centering 
  \begin{tabular}{c}
    \includegraphics[width=0.49\textwidth]{figures/analysis_2/Interpretation/TriggerEfficiency.png} 
  \end{tabular}
...
The ratio is then used to determine the event weights.
These event weights are applied on the simulated signal samples in this analysis and lead to changes in the signal prediction of 1.9 to 4.4\%, which are taken as systematic uncertainties.
...
%\CLsb &= \sum\limits_{0}^{n_{\text{obs}}}\text{Poisson}\left( n |\lambda = b+\mu\cdot s   \right),\\
%\CLb  &= \sum\limits_{0}^{n_{\text{obs}}}\text{Poisson}\left( n |\lambda = b   \right),
\end{aligned}
\end{equation}
...
For one Gaussian distributed source of systematic uncertainty on the background, this leads to the following expressions for the test statistics of $q_{s+b}$ and $q_b$
\begin{equation}
...
In case of multiple signal regions, the distribution of the systematic uncertainties becomes a multi-dimensional probability density function that takes the covariance matrix of the systematic uncertainties in different signal regions into account. FIXME
In order to get rid of the nuisance paramaters $\delta_b$ and $\delta_s$ either an integration over $\delta_b$ and $\delta_s$ can be done or a maximisation with respect to the nuisance paramaters can be done.
...
The confirmation of the excluded parameter space in~\cite{bib:CMS:DT_8TeV} is especially interesting since the signal regions of the two searches show little correlation for small lifetimes. FIXME
