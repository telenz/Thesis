...
The systematic uncertainties on the number of signal events in the four signal regions are caused by uncertainties in the generation and simulation of signal events and the integrated luminosity during the considered data-taking period.
...
The trigger uncertainty is assessed by comparing data to simulation differences of the trigger efficiency.
...
\begin{figure}[!b]
  \centering 
  \begin{tabular}{c}
    \includegraphics[width=0.43\textwidth]{figures/analysis/Interpretation/TriggerEfficiency.png} 
  \end{tabular}
  \caption{Trigger turn-on curves in data (black triangles) and simulation (red squares) as a function of PF \met (not taking into account the energy of the muons). Taken from~\cite{bib:CMS:DT_Thesis}}
...
The transverse momentum of all jets is corrected for non-uniformities in the energy response as a function of the jet $\eta$ and \pt and for data to simulation differences.
...
The JES correction is applied as a multiplicative factor on each jet transverse momentum contained in an event.
...
To cover both sources, a variation of the inelastic cross section by  $\pm 5\%$ is done according to the recommendation in~\cite{bib:CMS:PileupSysUnc}.
...
An uncertainty on the simulation of \ias needs to be estimated in order to account for possible data to simulation differences for highly ionising particles.
...
The kaon and proton lines are visible in both datasets. 
The deuteron line is only visible in data, as deuterons are not simulated.
...
It can be seen, that major uncertainties are the simulation of initial state radiation, of calorimeter isolation, and of \ias.
...
\begin{figure}[!h]
  ...
  \caption{Four different \CLs exclusion limits for charginos with mean lifetimes of 5\cm (top left), 10\cm (top right), 50\cm (bottom left), 1000\cm (bottom right).
           The black line depicts the expected 95\% confidence level (CL) upper cross-section limit with the 1-$\sigma$ (green band) and 2-$\sigma$ (yellow band) intervals.
           The red line is the observed limit.
...
\begin{figure}[!h]
  ...
  \caption{Excluded regions in the mass versus lifetime space.
           All excluded models are located left of the contour line.
            The black line depicts the expected 95\% CL upper cross-section limit with the 1-$\sigma$ (green band) and 2-$\sigma$ (yellow band) intervals.
           The red line is the observed limit.}
  \label{fig:2dLimit}
\end{figure} 
