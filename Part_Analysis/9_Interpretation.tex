...
\CLsb &= \sum\limits_{0}^{n_{\text{obs}}} \text{Poisson}\left( n |\lambda = b \cdot (1+\delta_b)+\mu\cdot s   \right) \text{Gauss}\left(\delta_b|\text{mean}=0, \sigma = \sigma_b\right),\\
\CLb  &= \sum\limits_{0}^{n_{\text{obs}}} \text{Poisson}\left( n |\lambda = b \cdot (1+\delta_b)   \right) \text{Gauss}\left(\delta_b|\text{mean}=0, \sigma = \sigma_b\right),
\end{split}
\end{equation}
